defmodule Animalshelter.Tickets do
  @moduledoc """
  The Tickets context.
  """
  ...
  """
  def create_ticket(%Animal{} = animal, %User{} = user, attrs \\ %{}) do
    %Ticket{animal: animal, user: user}
    |> Ticket.changeset(attrs)
    |> Repo.insert()
    |> case do
      {:ok, ticket} ->
        Animals.broadcast(animal.id, {:ticket_created, ticket})
        {:ok, ticket}
      {:error, changeset} ->
        {:error, changeset}
    end
